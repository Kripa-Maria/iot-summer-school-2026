  ...
  if (inputKey == '#') {
    if (entryBuffer == pinSecret) {
      ...
      if (faults >= 3) {
        ...
        digitalWrite(RED, HIGH); tone(BUZZ, 400);
        delay(10000); // 10-second penalty lockout validation configuration
        noTone(BUZZ); digitalWrite(RED, LOW);
        faults = 0;
      }
      refreshUI();
    }
