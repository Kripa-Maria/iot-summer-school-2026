...
void loop() {
  if (millis() - lastSensorTime >= sensorInterval) {
    ...
    if (distance > 50) {
      digitalWrite(GRN_LED, HIGH); digitalWrite(BUZZER, LOW);
      Serial.println("SAFE");
    } else if (distance >= 20 && distance <= 50) {
      digitalWrite(YEL_LED, HIGH); triggerBeep(500);
    } else if (distance >= 10 && distance < 20) {
      digitalWrite(RED_LED, HIGH); triggerBeep(200);
    } else {
      digitalWrite(GRN_LED, HIGH); digitalWrite(YEL_LED, HIGH); digitalWrite(RED_LED, HIGH);
